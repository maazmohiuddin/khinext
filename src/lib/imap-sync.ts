 ...
 *   • Default: the cPanel mailbox (mail.khinext.com) via the SMTP_* creds.
 ...
 *   IMAP_HOST   (fallback SMTP_HOST → "mail.khinext.com")
 *   IMAP_PORT   (default 993)
 *   IMAP_USER   (fallback SMTP_USER → "info@khinext.com")
...
export async function syncInboxEmails(): Promise<SyncResult> {
  ...
  const host = process.env.IMAP_HOST ?? process.env.SMTP_HOST ?? "mail.khinext.com";
  const port = Number(process.env.IMAP_PORT ?? 993);
  const user = process.env.IMAP_USER ?? process.env.SMTP_USER ?? "info@khinext.com";
  ...
  const ownAddresses = new Set(
    [user, process.env.SMTP_USER ?? "info@khinext.com"].map(a => a.toLowerCase()),
  );
