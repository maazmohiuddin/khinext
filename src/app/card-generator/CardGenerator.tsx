...
const isDragging   = useRef(false);
  ...
  async function handleShareWhatsApp() {
    ...
    setUploading(false);
    if (!card) return;
    const shareUrl = `${window.location.origin}/go/${encodeURIComponent(card.slug)}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(`Check out my Khinext '26 card! ${shareUrl}`)}`, "_blank", "noopener");
  }
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={handleShareLinkedIn} disabled={uploading || !isCardReady}
                  className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-white/12 hover:border-[#0077B5] hover:bg-[#0077B5]/10 transition-all text-xs text-white/45 hover:text-white font-medium disabled:opacity-40 disabled:cursor-not-allowed">
                  {uploading
                ...
                <button onClick={handleShareFacebook} disabled={uploading || !isCardReady}
                  className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-white/12 hover:border-[#1877F2] hover:bg-[#1877F2]/10 transition-all text-xs text-white/45 hover:text-white font-medium disabled:opacity-40 disabled:cursor-not-allowed">
                  {uploading
                    ? <Loader2 size={13} className="animate-spin" />
                    : <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
                ...
                <button onClick={handleShareWhatsApp} disabled={uploading || !isCardReady}
                  className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-white/12 hover:border-[#25D366] hover:bg-[#25D366]/10 transition-all text-xs text-white/45 hover:text-white font-medium disabled:opacity-40 disabled:cursor-not-allowed">
                  {uploading
                    ? <Loader2 size={13} className="animate-spin" />
                    : <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
                ...
                <button onClick={handleDownload} disabled={!isCardReady}
                  ...
                  className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-white/12 hover:border-[#E1306C] hover:bg-[#E1306C]/10 transition-all text-xs text-white/45 hover:text-white font-medium disabled:opacity-40 disabled:cursor-not-allowed">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
              ...
              <p className="text-[11px] text-white/22 text-center mt-2">
                LinkedIn, Facebook &amp; WhatsApp share a link preview · Instagram: download then post
              </p>
            </div>
          </div>
